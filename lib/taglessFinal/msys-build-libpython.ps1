...
$vendorDir = Join-Path $scriptDir "vendor-mingw"

& C:\msys64\usr\bin\bash -lc "cd '$vendorDir' && autoreconf -vfi && ./configure --prefix=/mingw64 --host=x86_64-w64-mingw32 --build=x86_64-w64-mingw32 --enable-shared --with-system-expat --with-system-ffi --with-system-libmpdec --without-ensurepip --enable-loadable-sqlite-extensions --with-tzpath=/mingw64/share/zoneinfo --enable-optimizations && make -j8 libpython3.11.a"